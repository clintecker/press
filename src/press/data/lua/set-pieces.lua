...
local TAIL_SIZES = {
  "\\normalsize", "\\normalsize", "\\small", "\\small", "\\footnotesize",
...
local function get_lines(div)
  for _, blk in ipairs(div.content) do
    if blk.t == "LineBlock" then return blk.content end
  end
  local lines = {}
  for _, blk in ipairs(div.content) do
    if blk.t == "Para" or blk.t == "Plain" then lines[#lines + 1] = blk.content end
  end
  return lines
end
...
local function with_tail(inlines, extra)
  ...
  for _, il in ipairs(extra) do out[#out + 1] = il end
  return out
end
...
local function cascade(lines)
  if FORMAT:match("latex") then
    ...
    for i, line in ipairs(lines) do
      local pre = pandoc.RawInline("latex",
        string.format("\\noindent\\hspace*{%.2fem}", (i - 1) * CASCADE_STEP))
      out[#out + 1] = pandoc.Plain(with_tail({ pre }, with_tail(line,
  ...
  for _, line in ipairs(lines) do
    if #line == 0 then
      ...
      if not first then
        ...
        if gap then inl[#inl + 1] = pandoc.LineBreak() end
      end
      for _, il in ipairs(line) do inl[#inl + 1] = il end
...
local function tail(lines)
  ...
  if FORMAT:match("latex") then
    ...
    for i, line in ipairs(lines) do
      ...
      local pre = pandoc.RawInline("latex",
        string.format("\\hspace*{%.2fem}{%s ", offset, size))
      out[#out + 1] = pandoc.Plain(with_tail({ pre }, with_tail(line,
        { pandoc.RawInline("latex", "}\\par") })))
    end
...
function Div(el)
  if el.classes:includes("cascade") then return cascade(get_lines(el)) end
  if el.classes:includes("verse") then return verse(get_lines(el)) end
  if el.classes:includes("tail") then return tail(get_lines(el)) end
  return nil
end
