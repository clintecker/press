...
for _, cp in ipairs({
  ...
  0x2012,
  0x2010,
  0x2D, -- dashes and hyphen
  0x00BF,
  0x00A1, -- inverted marks
}) do
  LEAD[cp] = true
end
...
local function split_initial(s)
  ...
  while i <= n and is_space(cps[i]) do
    i = i + 1
  end
  local lead_start = i
  while i <= n and LEAD[cps[i]] do
    i = i + 1
  end
  local lead_end = i - 1
  if i > n then
  ...
  local first = cps[i]
  if LEAD[first] or is_space(first) then
    return nil
  end
  local init_start = i
  i = i + 1
  while i <= n and is_combining(cps[i]) do
    i = i + 1
  end
  local init_end = i - 1
  -- remainder of the first word
  local word_start = i
  while i <= n and not is_space(cps[i]) do
    i = i + 1
  end
  local word_end = i - 1
  local rest_start = i
...
local function first_text(inlines)
  ...
  if
    (head.t == "Emph" or head.t == "Strong" or head.t == "Span" or head.t == "Quoted")
    and head.content
    and head.content[1]
    and head.content[1].t == "Str"
  then
...
local function drop_inlines(parts)
  ...
  if parts.lead ~= "" then
    out[#out + 1] =
      pandoc.Span(pandoc.Str(parts.lead), pandoc.Attr("", { "drop-cap-lead" }))
  end
  out[#out + 1] =
    pandoc.Span(pandoc.Str(parts.initial), pandoc.Attr("", { "drop-cap" }))
  if parts.word ~= "" then
    out[#out + 1] =
      pandoc.Span(pandoc.Str(parts.word), pandoc.Attr("", { "opening-word-rest" }))
  end
  return out
end
...
local function unwrap_leading_quote(content)
  ...
  if head == nil or head.t ~= "Quoted" then
    return content
  end
  ...
  if inner[1] == nil or inner[1].t ~= "Str" then
    return content
  end
...
local function transform(para)
  ...
  if parts.rest ~= "" then
    replacement[#replacement + 1] = pandoc.Str(parts.rest)
  end
  list[1] = nil
  local rebuilt = {}
  for _, el in ipairs(replacement) do
    rebuilt[#rebuilt + 1] = el
  end
  for k = 2, #list + 1 do
    if list[k] ~= nil then
      rebuilt[#rebuilt + 1] = list[k]
    end
  end
  -- write back
  for k = #list + 1, 1, -1 do
  ...
  if not FORMAT:match("latex") then
    ...
    if settings.style == "ornate" then
      classes[#classes + 1] = "ornate"
    end
