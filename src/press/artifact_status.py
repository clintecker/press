...
Evidence is a mapping from output path (relative to the book root) to
the digest that was verified; a build's verifiers write it, and the
desk reads it. Nothing here consults a clock.
"""
...
def output_paths(root: Path, slug: str, outputs: tuple[str, ...]) -> list[Path]:
    return [root / o.format(slug=slug) for o in outputs]


def artifact_state(root: Path, slug: str, outputs: tuple[str, ...],
    ...
    paths = output_paths(root, slug, outputs)
...
def record_evidence(root: Path, slug: str, outputs: tuple[str, ...]) -> dict[str, str]:
    """The digests to record as verified after a verifier passes: the
    current digest of every file output that exists."""

    evidence: dict[str, str] = {}
    for path in output_paths(root, slug, outputs):
        if path.is_file():
            evidence[str(path.relative_to(root))] = _digest(path)
    return evidence
