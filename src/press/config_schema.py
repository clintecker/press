...
REGISTRY: tuple[Field, ...] = (
    ...
    Field(
        ...
        METADATA,
        IMMUTABLE,
        help="page trim; derived from print.profile",
    ...
    _w(
        "commerce.print-ordering.storefront-url",
        METADATA,
        help=_URL,
        https=True,
        secret_guarded=True,
    ...
    _w(
        "commerce.print-ordering.seller-of-record",
        METADATA,
        help="who the reader buys from",
        secret_guarded=True,
    ),
    _w(
        "commerce.print-ordering.support-url",
        METADATA,
        help=f"support page {_URL} (optional; omit to generate)",
        https=True,
        secret_guarded=True,
    ),
    _w(
        "commerce.print-ordering.privacy-url",
        METADATA,
        help=f"privacy page {_URL} (optional; omit to generate)",
        https=True,
        secret_guarded=True,
    ),
    _w(
        "commerce.print-ordering.refund-url",
        METADATA,
        help=f"returns page {_URL} (optional; omit to generate)",
        https=True,
        secret_guarded=True,
    ),
    _w(
        "commerce.print-ordering.policies.support",
        METADATA,
        help="your own text appended to the generated support page",
        secret_guarded=True,
    ),
    _w(
        "commerce.print-ordering.policies.privacy",
        METADATA,
        help="your own text appended to the generated privacy page",
        secret_guarded=True,
    ),
    _w(
        "commerce.print-ordering.policies.refund",
        METADATA,
        help="your own text appended to the generated returns page",
        secret_guarded=True,
    ),
    # config/house-rules.yaml
    _w(
        "banned-patterns", HOUSE_RULES, "mapping", help="regex -> label; applied by the style audit"
    ),
    ...
    _w("cover.subject", AESTHETIC, help="the subject of the cover illustration, one phrase"),
    ...
    _w("plates.style", AESTHETIC, help="default illustration style id (default wood-engraving)"),
    ...
    _w(
        "scene-break",
        AESTHETIC,
        help="thematic-break ornament (rule, asterism, or fairy-dust)",
        choices=("rule", "asterism", "fairy-dust"),
    ),
...
def _validate_house_rules(root: Path, proposed: object) -> list[str]:
    ...
    if banned is not None and not isinstance(banned, dict):
        problems.append(
            f"banned-patterns must be a mapping of regex -> label, not {_shape_name(banned)}"
        )
...
def _validate_front_matter(root: Path, proposed: object) -> list[str]:
    ...
    if epigraph is not None and not isinstance(epigraph, dict):
        return [
            f"epigraph must be a mapping with quote and attribution, not {_shape_name(epigraph)}"
        ]
    return []
...
def _validate_index_terms(root: Path, proposed: object) -> list[str]:
    ...
    for index, entry in enumerate(proposed, start=1):
        if not isinstance(entry, dict):
            problems.append(
                f"entry {index}: must be a mapping with term and match, not {_shape_name(entry)}"
            )
            continue
        term = entry.get("term")
        ...
        if not isinstance(match, list) or not match:
            problems.append(f"entry {index}: match must be a non-empty list of strings")
...
def _validate_authorities(root: Path, proposed: object) -> list[str]:
    ...
    for index, entry in enumerate(proposed, start=1):
        if not isinstance(entry, dict):
            problems.append(f"entry {index}: must be a mapping, not {_shape_name(entry)}")
            continue
