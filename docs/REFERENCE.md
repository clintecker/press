...
print pack   print  verify-print  coverwrap  publish kdp|ingram [--report-only]  isbn status | assign print|epub  onix [--forthcoming]  pcn  lookup lccn|issn [<number>]
utilities    render  wordcount  figures  clean  new <directory>  add chapter|appendix <name> [--front]  config get|set|unset|list|validate  selftest  doctor  migrate [plan] | apply | rollback | status
instruments  skills  workflows  desk
art          art commission [targets] | accept <file> --as <target>  cover [--style <id>] [--subject <text>] | --list  illustrate <name> [--style <id>] [--from <image>] | --list
