...
def _receipt(
    layer, prereqs=None, proofs=("INV-config-slug",), clean=True, inputs=None, commit="abc123"
):
...
def _full_chain(inputs, commit="c", clean=True, package="PKG"):
    ...
    for layer in receipts.LAYERS[:-1]:
        chain.append(
            _receipt(
                layer, prereqs=chain[-1:], proofs=[], clean=clean, inputs=inputs, commit=commit
            )
        )
    release = receipts.Receipt(
        ...
        tree_clean=clean,
        inputs=inputs,
        prerequisites=[r.digest() for r in chain],
        proofs=[],
...
def _clean_ci(monkeypatch, commit="c"):
    ...
    monkeypatch.setattr(
        receipts, "current_inputs", lambda toolchain_digest="unpinned": (inputs, commit, True)
    )
    return inputs
...
def test_ci_release_fails_closed_when_a_job_did_not_run(monkeypatch):
    ...
    assert any(
        "missing tier receipt 'integration'" in p for p in receipts.verify_ci_release(chain, "PKG")
    )
...
def test_ci_release_fails_when_a_receipt_is_from_another_commit(monkeypatch):
    ...
    assert any("disagree on source commit" in p for p in receipts.verify_ci_release(chain, "PKG"))
...
def test_ci_release_fails_on_a_package_mismatch(monkeypatch):
    ...
    chain = receipts.assemble_release(_tiers(inputs), "PKG")
    assert any(
        "package does not match" in p for p in receipts.verify_ci_release(chain, "DIFFERENT")
    )
...
def test_verify_release_refuses_a_dirty_tree():
    ...
    inputs = {
        ...
        "scenarios": "d",
        "surfaces": "d",
        "toolchain": receipts.pinned_toolchain_digest(),
    }
    release = receipts.Receipt(
        schema_version=receipts.SCHEMA_VERSION,
        layer="release",
