...
AES = {
    "book-colors": {"ink": "#12333a"},
    "web-palette": {"paper": "#eef4f2"},
...
def test_build_prompt_fills_subject_and_guards_single_ink():
    ...
    prompt = illustrate.build_prompt(
        styles["wood-engraving"], illustrate.context(AES), "a tide pool"
    )
...
def test_subject_is_the_figures_art_description_not_its_caption():
    subject = illustrate.subject_from_figure(
        _fig(
            caption="A compositor at the case",
            description="a compositor's left hand on a brass composing stick",
        ),
        "x",
    )
