...
def test_with_a_base_every_url_shaped_tag_is_emitted():
    ...
    head = webmeta.head_fragment(
        ...
        og_type="book",
        site_name="Press",
        canonical="https://me.test/",
        image="https://me.test/cover.jpg",
        image_width=800,
        image_height=1200,
        image_alt="Cover of A Book",
        extra_properties=[("book:author", "A. Author")],
        jsonld=node,
    )
...
def test_without_a_base_nothing_url_shaped_survives():
    ...
    head = webmeta.head_fragment(
        ...
        description="One line.",
        og_type="article",
        canonical="https://me.test/1.html",
        image="https://me.test/cover.jpg",
        image_width=800,
        image_height=1200,
        jsonld=node,
...
def test_description_meta_is_opt_in():
    with_meta = webmeta.head_fragment(base="", title="T", description="D", description_meta=True)
...
def test_values_are_html_escaped():
    head = webmeta.head_fragment(base="", title='A & "B"', description="x <y>")
...
def test_each_cell_takes_its_own_column_header():
    out = webmeta.label_table_cells(_headed("<tr><td>Detect</td><td>turns RF into audio</td></tr>"))
...
def test_a_quote_in_a_header_cannot_break_out_of_the_attribute():
    out = webmeta.label_table_cells(_headed("<tr><td>x</td></tr>", heads='<th>The "big" one</th>'))
...
def test_extra_cells_beyond_the_headers_stay_bare():
    ...
    out = webmeta.label_table_cells(_headed("<tr><td>A</td><td>b</td><td>surplus</td></tr>"))
...
def test_two_tables_on_one_page_get_their_own_headers():
    ...
    second = _headed("<tr><td>B</td><td>b</td></tr>", heads="<th>Reading</th><th>Instrument</th>")
